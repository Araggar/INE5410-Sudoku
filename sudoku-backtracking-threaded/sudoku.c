...
void* sudoku_solver(void* args){
	...
	fflush(stdout);
	int sudo_r[size_qd];
	deep_copy(size, sudo_r, ((Data*)args)->state);
				...
				for(int n = sudo_r[ind]+1; n<size_sq+1; n++){
					if(valid_in_row(sudo_r,n,ind,size_sq) & valid_in_col(sudo_r,n,ind,size_sq) & valid_in_sq(sudo_r,n,ind,size)){
						if(next){
							...
							deep_copy(size, temp_sudoku, sudo_r);
...
int main(){
	...
	for(int n = 1; n<17; n++){
		if(valid_in_row(sudo,n,0,16) & valid_in_col(sudo,n,0,16) & valid_in_sq(sudo,n,0,4)){
			datas[n].state = sudo;
			datas[n].size = 4;
			datas[n].start_value = n;
			datas[n].start_ind = 0;
			printf("%Thread %u \n",datas[n].start_value);
			fflush(stdout);
			stackpush(&s, &datas[n]);
